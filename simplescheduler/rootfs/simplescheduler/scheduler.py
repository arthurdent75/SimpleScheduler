    ...
    while True:
        ...
        if seconds == '00':
            ...
            if current_time == "00:01" or sunset == "" or sunrise == "":
                if options['debug']: main.printlog('DEBUG: Retrieving sunrise and sunset')
            ...
            if options['debug']: main.printlog("DEBUG: Queue length: %d " % len(command_queue))
            for key in command_queue.copy():
                ...
                if entity_status == 'unavailable':
                    attempt = 1 + max_retry - int(value['countdown'])
                    main.printlog("SCHED: [%s] is unavailable. Attempt %d of %d" % (
                    friendly_name.get(value['entity_id'], value['entity_id']), attempt, max_retry))
