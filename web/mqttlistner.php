...
function refresh_switches($mqtt_obj, bool $clear = false ) {
	...
	$payload_template='{"name": "simplescheduler_###_@@@" , "icon":"mdi:calendar-clock" , "command_topic": "homeassistant/switch/simplescheduler/###/set", "state_topic": "homeassistant/switch/simplescheduler/###/state"}';

	foreach ($sched as $s) {
		$topic=str_replace('###',$s->id,$config_topic_template);
		$payload=str_replace('###',$s->id,$payload_template);
		$payload=str_replace('@@@',$s->name,$payload);
